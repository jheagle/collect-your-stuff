...
describe('TaskQueue', () => {
  ...
  test('can store elements', () => {
    ...
    const someQueue = TaskQueue.fromArray(arrayData)
  ...
  test('can add to the queue', () => {
    const someQueue = TaskQueue.fromArray(['one', 'two', 'three', 'four'])
  ...
  test('can remove from the queue', () => {
    const someQueue = TaskQueue.fromArray(['one', 'two', 'three', 'four', 'five', 'six'])
    expect(someQueue.size()).toBe(6)
    someQueue.remove()
    expect(someQueue.size()).toBe(5)
    someQueue.remove()
  ...
  test('returns success when no queueable tasks', () => {
    const someQueue = TaskQueue.fromArray([])
  ...
  test('returns assigned queue task', () => {
    ...
    const someQueue = TaskQueue.fromArray(willRunTasks)
    expect(someQueue.size()).toBe(4)
    someQueue.peek().ready = true
  ...
  test('returns error when dequeue has nothing ready', () => {
    ...
    const someQueue = TaskQueue.fromArray(queueItems.map(item => ({ task: item, ready: false })))
    expect(someQueue.size()).toBe(4)
    const result = someQueue.dequeue()
    expect(result.success).toBeFalsy()
