...
describe('TaskStack', () => {
  test('can store elements', () => {
    ...
    const someStack = TaskStack.fromArray(arrayData)
  ...
  test('can add to the stack', () => {
    const someStack = TaskStack.fromArray(['two', 'three', 'four', 'five'])
    someStack.push('one')
  ...
  test('can remove from the stack', () => {
    const someStack = TaskStack.fromArray(['one', 'two', 'three', 'four', 'five', 'six'])
    expect(someStack.size()).toBe(6)
    someStack.remove()
    expect(someStack.size()).toBe(5)
    someStack.remove()
  ...
  test('returns success when no stackable tasks', () => {
    const someStack = TaskStack.fromArray([])
  ...
  test('returns assigned stack data', () => {
    const someStack = TaskStack.fromArray(['one', 'two', 'three', 'four'])
    expect(someStack.size()).toBe(4)
    const result = someStack.pop()
  ...
  test('returns assigned stack task', () => {
    ...
    const someStack = TaskStack.fromArray(taskData)
    expect(someStack.size()).toBe(4)
  ...
  test('fromArray keeps the classes it was given', () => {
    ...
    const someStack = TaskStack.fromArray(['a', 'b'], CustomStackable)
